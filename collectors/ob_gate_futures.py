...
# -*- coding: utf-8 -*-
"""
Gate.io Futures (USDT Perpetual) — order book collector (10 levels).
WS: fx-ws.gateio.ws/v4/ws/usdt  Channel: futures.order_book
Redis key: ob:gt:f:{SYMBOL}
...
- Custom ping: {"time": ts, "channel": "futures.ping"} every 20s
- futures.order_book sends periodic snapshots (event="all" initial, then "update")
- Bids/asks format: [{"p": price_str, "s": size_num}, ...]  (dict-based)
  with fallback for list format [["price", "qty"], ...] if exchange changes protocol
- result.contract = native symbol (BTC_USDT)
...
def parse_levels(raw: list) -> list[tuple[str, str]]:
    """
    Parse bids/asks entries — Gate futures uses dict format {"p": price, "s": size}.
    Falls back to list format [price, qty] if protocol differs.
    Returns list of (price_str, qty_str).
    """
    result = []
    for row in raw:
...
def build_mapping(bids: list, asks: list, ts_ms: bytes) -> dict:
    m = {b"t": ts_ms}
    for i, (p, q) in enumerate(bids[:LEVELS], 1):
        m[f"b{i}".encode()]  = p.encode()
        m[f"bq{i}".encode()] = q.encode()
    for i, (p, q) in enumerate(asks[:LEVELS], 1):
        m[f"a{i}".encode()]  = p.encode()
        m[f"aq{i}".encode()] = q.encode()
    return m
        ...
        async for raw in ws:
            ...
            try:
                ...
                if msg.get("channel") != CHANNEL:
                    continue
                # Accept both "all" (initial snapshot) and "update" (periodic snapshot)
                if msg.get("event") not in ("all", "update"):
                    continue
                result = msg.get("result", {})
                # Gate futures: contract field holds native symbol
                native_sym = result.get("contract", "") or result.get("s", "")
                raw_bids   = result.get("bids", [])
                raw_asks   = result.get("asks", [])
                if native_sym and (raw_bids or raw_asks):
