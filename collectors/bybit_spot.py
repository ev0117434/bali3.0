...
WS: stream.bybit.com/v5/public/spot  Channel: orderbook.1
Redis key: md:bb:s:{SYMBOL}  Fields: b (bid), a (ask), t (exchange ts ms)

Protocol notes:
- Single connection, subscribe in batches of 10 (spot frame limit)
- Custom ping: {"op": "ping"} every 20s
- Snapshot has full b/a; delta may have empty b or a (partial update)
"""
...
SUB_BATCH       = 10      # max args per subscribe message (spot)
BATCH_SIZE      = 100
BATCH_TIMEOUT   = 0.020
PING_INTERVAL   = 20
STATS_INTERVAL  = 30
    ...
    try:
        async for raw in ws:
            ...
            try:
                ...
                if len(fields) > 1:   # at least one price field
