...
# -*- coding: utf-8 -*-
"""
Bitget Futures (USDT Perpetual) — order book collector (10 levels).
WS: ws.bitget.com/v2/ws/public  Channel: books15 (up to 15 levels)
Redis key: ob:bg:f:{SYMBOL}
...
- action="snapshot" → full order book replacement, initialize local state
- action="update"   → incremental delta; qty="0" means remove level
- Local book reset on every reconnect (fresh snapshots follow automatically)
- Top-10 levels extracted by sorting price desc (bids) / asc (asks)
...
async def main() -> None:
    ...
    books: dict = {}   # sym -> {"bids": {price: qty}, "asks": {price: qty}}
