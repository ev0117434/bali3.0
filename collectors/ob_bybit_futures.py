...
# -*- coding: utf-8 -*-
"""
Bybit Futures (Linear USDT Perpetual) — order book collector (10 levels).
WS: stream.bybit.com/v5/public/linear  Channel: orderbook.50
Redis key: ob:bb:f:{SYMBOL}
Fields: b1..b10 (bid prices), bq1..bq10 (bid qty),
        a1..a10 (ask prices), aq1..aq10 (ask qty), t (exchange ts ms)

Protocol notes:
- Single connection, subscribe in batches of 200 (linear allows larger batches)
- Custom ping: {"op": "ping"} every 20s
- type="snapshot" → full book (logged per symbol on first init)
- type="delta"    → incremental; WARN if arrives before snapshot
- Local book reset on every reconnect
...
Counters logged in stats (every 30s):
  ...
  delta_skipped      — deltas dropped (no snapshot yet)
  avg_levels         — average levels per side actually present
