...
# Load fixed source IDs: binance_spot=0, binance_futures=1, ...
_SOURCE_IDS_PATH = Path(__file__).parent / "source_ids.json"
SOURCE_IDS: dict[str, int] = {
    k: v for k, v in json.loads(_SOURCE_IDS_PATH.read_text()).items()
    if not k.startswith("_")
}

# Load symbol IDs: BTCUSDT=0, ETHUSDT=1, ... (generated by dictionaries/main.py)
_SYMBOL_IDS_PATH = Path(__file__).parent / "symbol_ids.json"
SYMBOL_IDS: dict[str, int] = (
    json.loads(_SYMBOL_IDS_PATH.read_text())
    if _SYMBOL_IDS_PATH.exists()
    else {}
)
...
FR_EMPTY  = ","                           # 1 comma → 2 empty CSV fields (fr_r,fr_ft)
...
SNAPSHOT_CSV_HEADER = (
    f"spot_source_id,fut_source_id,symbol_id,ask_spot,bid_futures,spread_pct,ts,"
...
async def write_snapshot_history(
    ...
    # FR history for futures exchange — best-effort; missing seconds get empty fields
    fr_hist = await read_fr_history(r, fut_ex, sym, since_sec)
    ...
    for ts_sec in sorted(set(spot_ask_by_sec) & set(fut_bid_by_sec)):
        ...
        fr   = fr_hist.get(ts_sec)
        fr_s = f"{fr[0]},{fr[2]}" if fr else FR_EMPTY
        row = (
